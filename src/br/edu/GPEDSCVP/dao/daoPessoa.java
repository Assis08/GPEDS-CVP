...
public class daoPessoa {
    ...
    public boolean verificaExclusao(Pessoa pessoa) {
        Integer id_pessoa = 0;
        boolean excluir = false;

        conecta_banco.executeSQL("select * from certificacao where id_pessoa = " + pessoa.getId_pessoa() + " and certificacao.in_ativo = 'A'");
        try {
            conecta_banco.resultset.first();
            id_pessoa = conecta_banco.resultset.getInt("id_pessoa");
            return false;
        } catch (SQLException ex) {
            excluir = true;
        }

        conecta_banco.executeSQL("select * from fornecimento where id_pessoa = " + pessoa.getId_pessoa() + " and fornecimento.in_ativo = 'A'");
        try {
            conecta_banco.resultset.first();
            id_pessoa = conecta_banco.resultset.getInt("id_pessoa");
            return false;
        ...
        conecta_banco.executeSQL("select * from fornecedor_componente where id_pessoa = " + pessoa.getId_pessoa() + " and fornecedor_componente.in_ativo = 'A'");
        try {
            conecta_banco.resultset.first();
            id_pessoa = conecta_banco.resultset.getInt("id_pessoa");
            return false;
